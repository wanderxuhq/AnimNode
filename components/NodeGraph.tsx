...
export const NodeGraph: React.FC<NodeGraphProps> = ({ project, onSelect }) => {
  ...
  return (
    <div ref={containerRef} className="flex-1 bg-zinc-950 relative overflow-hidden bg-[radial-gradient(#27272a_1px,transparent_1px)] [background-size:16px] group/graph pt-12">
            ...
            >
                ...
                <div className="p-3 space-y-2 bg-zinc-900/90">
                     {Object.entries(node.properties).slice(0, 5).map(([key, rawProp]) => {
                        ...
                        const displayValue = !isExpression && prop.type !== 'ref'
                            ? (typeof prop.value === 'number' ? prop.value.toFixed(1) : String(prop.value))
                            : '...';
                        ...
                        <div key={key} className="flex justify-between items-center text-[10px] h-5 border-b border-zinc-800/50 last:border-0">
                             ...
                             <span className={`font-mono truncate max-w-[120px] ${isExpression ? 'text-blue-400' : 'text-zinc-400'}`}>
                                {isExpression ? 'ƒ(t)' : displayValue}
                             </span>
                        </div>
                     )})}
                </div>
