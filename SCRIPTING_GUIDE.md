...
*   **中心锚点**: 系统的默认行为是**中心锚点 (Center Anchor)**，这与 AE 和 Houdini 的 Primitive 行为一致。
    *   `x` 和 `y` 属性代表物体的**几何中心**。
    *   **矩形 (Rect)**: `(x, y)` 是矩形的**中心点**。
    *   **圆形 (Circle)**: `(x, y)` 是圆的**圆心**。
    *   **矢量 (Vector)**: `(x, y)` 是 SVG Path 坐标系的原点。绘制的路径点是相对于此原点的。
...
| `x` | `number` | RW | 400 | **中心点** X 轴位置 (px)。 |
| `y` | `number` | RW | 300 | **中心点** Y 轴位置 (px)。 |
